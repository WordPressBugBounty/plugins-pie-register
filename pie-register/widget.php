...
class Pie_Register_Widget extends WP_Widget
{
	...
	public function widget($args, $instance)
	{
		...
		if (is_user_logged_in() && !is_admin() && $option['redirect_user'] == 1) {
			...
			// FIX: Line 41 — $args['before_widget'] must be escaped
			echo wp_kses_post($args['before_widget']);

			echo ('<div class="piereg_container pieregWrapper">');
			if (isset($this->pie_post_array['success']) && $this->pie_post_array['success'] != "")
				// FIX: Line 45 — apply_filters output must be escaped
				echo ('<p class="piereg_message">' . wp_kses_post(apply_filters('piereg_messages', $this->pie_post_array['success'])) . '</p>');

			if (isset($this->pie_post_array['error']) && $this->pie_post_array['error'] != "")
				// FIX: Line 48 — apply_filters output must be escaped
				echo ('<p class="piereg_login_error">' . wp_kses_post(apply_filters('piereg_messages', $this->pie_post_array['error'])) . '</p>');

			if (isset($this->pie_post_array['registration_success']) && $this->pie_post_array['registration_success'] != "") {
				// FIX: Line 51 — apply_filters output must be escaped
				echo ('<p class="piereg_message">' . wp_kses_post(apply_filters('piereg_messages', $this->pie_post_array['registration_success'] )) . '</p>');
				unset($_POST);
			}
			...
			if (isset($errors->errors) && sizeof($errors->errors) > 0) {
				...
				if (isset($error) && !empty($error))
					// FIX: Line 63 — apply_filters output must be escaped
					echo ('<p class="piereg_login_error">' . wp_kses_post(apply_filters('piereg_messages', $error)) . '</p>');
			}
			echo wp_kses($pie_register->outputRegForm(true, $form_id, $form_title, $form_desc), $pie_register->piereg_forms_get_allowed_tags());
			echo ('</div>');
			// FIX: Line 67 — $args['after_widget'] must be escaped
...
class Pie_Login_Widget extends WP_Widget
{
	...
	public function widget($args, $instance)
	{
		...
		// FIX: Line 164 — $args['before_widget'] must be escaped
		echo wp_kses_post($args['before_widget']);
		...
		if (!is_user_logged_in()) {
			if (! empty($before_title))
				// FIX: Lines 171 — $args['before_title'], $before_title, $args['after_title'] must be escaped
				echo wp_kses_post($args['before_title']) . esc_html($before_title) . wp_kses_post($args['after_title']);
			...
			if (! empty($after_title))
				// FIX: Lines 186 — $args['before_title'], $after_title, $args['after_title'] must be escaped
				echo wp_kses_post($args['before_title']) . esc_html($after_title) . wp_kses_post($args['after_title']);
			...
			echo '</h4>';
			// FIX: Line 218 — __() output must be escaped with esc_html()
			echo '<a href="' . esc_url(wp_logout_url()) . '" class="logout-link" title="Logout">' . esc_html__("Logout", "pie-register") . '</a></div></div>';
		}
		// FIX: Line 220 — $args['after_widget'] must be escaped
		echo wp_kses_post($args['after_widget']);
	}
	// Widget Backend 
	public function form($instance)
...
class Pie_Forgot_Widget extends WP_Widget
{
	...
	public function widget($args, $instance)
	{
		...
		if (is_user_logged_in() && !is_admin() && $option['redirect_user'] == 1) {
			...
			// FIX: Line 293 — $args['before_widget'] must be escaped
			echo wp_kses_post($args['before_widget']);
			if (! empty($title))
				// FIX: Lines 295 — $args['before_title'] and $args['after_title'] must be escaped
			...
			// FIX: Line 300 — $args['after_widget'] must be escaped
			echo wp_kses_post($args['after_widget']);
		}
	}
	// Widget Backend 
