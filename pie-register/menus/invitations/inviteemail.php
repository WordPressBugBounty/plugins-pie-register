          ...
          <div class="cols-3">
            <select required id="pie_email_invitecode" name="pie_email_invitecode">
              ...
              foreach ($inviteCodes as $key => $val) {
                $expiryTime = gmdate("Y-m-d", strtotime($inviteCodes[$key]->expiry_date));
                $selected = selected($piereg['pie_email_invitecode'] == $inviteCodes[$key]->name, true, false);
                if (($inviteCodes[$key]->expiry_date == "0000-00-00" || $expiryTime > $today) && ($inviteCodes[$key]->code_usage == 0 || ($inviteCodes[$key]->code_usage > 0 && intval($inviteCodes[$key]->code_usage) > intval($inviteCodes[$key]->count)))) {
                  echo "<option value='" . esc_attr($inviteCodes[$key]->name) . "' " . esc_html($selected) . ">" . esc_html($inviteCodes[$key]->name) . "</option>";
                }
              }
              ?>
            </select>
      ...
      <li class="clearfix">
        <div class="fields extra-margin">
          <div class="cols-3">
            ...
            <span class="quotation import-email-invites">
              <?php
              $example_url = esc_url(plugin_dir_url(__FILE__) . '../examples/example-import-email-addresses-test.csv');
              $link = sprintf('<a target="_blank" download href="%s">%s</a>', $example_url, esc_html__('this example of the CSV file.', 'pie-register'));
              echo wp_kses(
                sprintf(
                  /* translators: %s: Example CSV file link */
                  __('You may want to see %s. Use a separate line for each email address.', 'pie-register'),
                  $link
                ),
                array(
                  'a' => array(
