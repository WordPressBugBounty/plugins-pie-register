            ...
            <li class="no-margin clearfix">
              <div class="fields">
                <p>
                  ...
                  <br />
                  <strong><?php esc_html_e("Note", 'pie-register') ?> :</strong> <?php esc_html_e("You must add the invitation code field to your registration form.", 'pie-register') ?>
                </p>
              </div>
            </li>
          </ul>
          <div id="piereg-detailed-guide-link">
            <a target="_blank" href="https://pieregister.com/documentation/how-to-use-invitation-codes-with-pie-register/"><?php esc_html_e("Learn how to use Invitation Codes", "pie-register"); ?></a>
          </div>
        </form>

        <div class="invite-tabs clearfix">
          <ul>
            <li <?php if (!isset($_GET['autogenerate']) && !isset($_GET['inviteemail']) && !isset($_GET['allowuserinvitation']) && !isset($_GET['usertracker'])) {
                  echo 'class="invite-active"';
                } ?>><a href="admin.php?page=pie-invitation-codes&rawcode">Raw Codes</a></li>
            <li <?php if (isset($_GET['autogenerate'])) {
                  echo 'class="invite-active"';
                } ?>><a href="admin.php?page=pie-invitation-codes&autogenerate">Auto Generate Codes</a></li>
            <li <?php if (isset($_GET['inviteemail'])) {
                  echo 'class="invite-active"';
                } ?>><a href="admin.php?page=pie-invitation-codes&inviteemail">Invite Through Email</a></li>
            <li <?php if (isset($_GET['allowuserinvitation'])) {
                  echo 'class="invite-active"';
                } ?>><a href="admin.php?page=pie-invitation-codes&allowuserinvitation">Allow Users to Invite</a></li>
            <li <?php if (isset($_GET['usertracker'])) {
                  echo 'class="invite-active"';
                } ?>><a href="admin.php?page=pie-invitation-codes&usertracker">User Tracker</a></li>
          </ul>
        </div>
        <?php
          ...
          <form method="post" id="form_invitation_code_per_page_items">
            ...
            <select name="invitation_code_per_page_items" id="invitation_code_per_page_items" title="<?php esc_attr_e("Invitation codes to show on a page.", "pie-register"); ?>">
              ...
              for ($per_page_item = 10; $per_page_item <= 50; $per_page_item += 10) {
                $checked = selected($per_page == $per_page_item, true, false);
                echo '<option value="' . esc_attr($per_page_item) . '" ' . esc_html($checked) . '>' . esc_html($per_page_item) . '</option>';
              }
