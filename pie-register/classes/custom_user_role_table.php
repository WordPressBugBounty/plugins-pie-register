...
class Pie_Custom_Role_Table extends WP_List_Table
{
    ...
    public function prepare_items( )
    {
        ...
        foreach ( $posts as $key => $post )
        {
            ...
            {
                $onclick = "confirmDelUserRole('".esc_js($post->id)."','".esc_js($custom_role)."','".esc_js($custom_role_key)."')";    
            }

			$posts[ $key ]->action = '<a class="delete" href="javascript:;" onclick="'.$onclick.'" title="'.esc_attr__("Delete","pie-register").'"></a>';
			$id++;
        }
